...
class App extends React.Component {
    ...
    getLiveScore(id) {
        cricLive.getLiveScore(id)
            .then(liveMatchScore => {
                console.log('Live score obj', liveMatchScore);
    ...
    render() {
        ...
        for (let i = 0; i < currentMatch.length; i++) {
            if (currentMatch[i].teams) {
                const countryList = Object.values(currentMatch[i].teams).map(country => country.name);
                const title = `${countryList[0]} vs ${countryList[1]}`;
                        ...
                        <AccordionItemTitle>
                            <div className="single-match-container" key={i}>
                                ...
                                <div className="match-title">
                                    {new Date(parseInt(`${currentMatch[i].startTime}000`)).toLocaleString()} - {title} ({currentMatch[i].type})
                                </div>
